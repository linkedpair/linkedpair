...
describe("aiBuddyMatch Algorithm", () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });

    it("should return a match of the same gender", async () => {
        const result = await aiBuddyMatch(mockCurrentUser);
        expect(result.match.gender).toBe("Male")
    });
    it("should return a match of the same faculty", async () => {
        const result = await aiBuddyMatch(mockCurrentUser);
        expect(result.match.faculty).toBe("Business")
    });
    ...
    it("should not return a match if there are no users of the same gender", async () => {
    ...
    it("should not return a match if there are no users of the same faculty", async () => {
        getDocs.mockResolvedValueOnce({
            forEach: (callback) => {
                callback({ data: () => ({
                    uid: "user2",
